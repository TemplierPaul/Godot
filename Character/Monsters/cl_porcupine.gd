...
func set_deck():
	...
	for i in range(3):
		new_card = Lifesteal_card.new("CHOMP", 0, 30, 1)
		new_card.frame.set_sprite("res://Character/Monsters/porcupine.png", true, 0.4)
		new_card.show_cost=false
		new_card.deck = self
		cards.append(new_card)
	for i in range(2):
		new_card = Heal_card.new("Grass Break", 0, 50)
		new_card.frame.set_sprite("res://Card/Card types/grass.png", true, 0.25)
		new_card.show_cost=false
		new_card.deck = self
		cards.append(new_card)
	return cards
